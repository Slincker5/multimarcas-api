...
def editar_celdas_desde_json(archivo_excel, json_data, user_uuid):
    ...
    hoja_insertar = workbook['DIgitar Aca Las Ofertas']
    ...
    for i, item in enumerate(json_data, start=3):
        ...
        hoja_insertar['C' + str(i)].value = descuento
    ...
    nombre_archivo = 'AFICHES-DESCUENTO-' + fecha_hora_actual.strftime("%d-%m-%Y-%H%M%S") + '.xlsx'
...
if __name__ == "__main__":
    if len(sys.argv) < 4:  # Se debe pasar la URL, TOKEN y USER_UUID como argumentos
        ...
        if data:
            archivo_excel = 'public/documentos/OFERTAS_MINI_DESC.xlsx'  # Reemplazar con la ruta real del archivo Excel
